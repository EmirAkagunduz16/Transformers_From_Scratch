...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        # Tokenizer'ı WordLevel modeli ile başlat ve [UNK] token'ını ekle
        tokenizer = Tokenizer(models.WordLevel(unk_token="[UNK]"))
        tokenizer.pre_tokenizer = Whitespace()
        
        # Tokenizer'ı veriseti üzerinde eğit
    ...
    # # Kelime hazinesini kontrol et ve [UNK] token'ının eklendiğini doğrula
    # vocab = tokenizer.get_vocab()
    # if "[UNK]" not in vocab:
    #     raise ValueError("[UNK] token is missing from the vocabulary after training/loading.")
...
def train_model(config):
  ...
  for epoch in range(initial_epoch, config['num_epochs']):
    batch_iterator = tqdm(train_data_loader ,desc=f'Processing epoch {epoch:.2d}')
    ...
    for batch in batch_iterator:
      ...
      decoder_output = model.encode(encoder_output, encoder_mask, decoder_input, decoder_mask) # (B, seq_len, d_model)
